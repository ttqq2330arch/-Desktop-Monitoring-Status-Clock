...
## 4. USB 转串口驱动（CH340 / CH341）

本仓库在 `drivers/CH340/` 下附带 **WCH（南京沁恒微电子）官方的 CH341SER 驱动**，
用于 Windows 上识别板载的 CH340C USB 转串口芯片。目录内容：

- `CH341SER.EXE` —— WCH 官方安装程序（兜底，双击或 `CH341SER.EXE /S` 静默安装）
- `install_driver.bat` —— **一键安装脚本（推荐）**：以管理员运行后，用 `pnputil` 把驱动
  预注册进系统驱动库，**之后任何 CH340 板子插上都会自动匹配，零点击**
- `raw/` —— 从本机已验证系统提取的原始驱动文件（`ch341ser.inf` / `CH341S64.SYS` /
  `CH341SER.CAT` / `CH341PT.DLL` / `CH341PTA64.DLL` / `CH341PORTSA64.DLL`，均带 WHQL
  微软交叉签名），供 `pnputil` 预注册使用；与 `CH341SER.EXE` 同属 WCH 驱动，许可一致
- `安装说明.txt` —— 简短安装提示（已并入 `README.md`，保留兼容）

- **为什么带它**：很多人的电脑没有预装该驱动，插上开发板后设备管理器里只会显示
  带黄色感叹号的未知设备，串口都认不出，自然跑不起来。带上驱动能让 clone 下来的人
  **双击 `CH341SER.EXE` 或用 `install_driver.bat` 一键预注册就识别**，免去到处找驱动的麻烦。
- **许可**：CH341SER 是 WCH 的专有驱动软件，随其 USB 转串口芯片免费分发，
  用于让芯片在主机上正常工作。具体许可条款以安装包内自带说明为准。本仓库仅作
  **随硬件分发的便利副本**提供，不包含任何修改。
- `CH340` 与 `CH341` 驱动通用（文件名虽是 CH341SER，CH340 开发板同样适用）。
- 如果你在 Linux / macOS 上使用，不需要这个驱动（系统内核自带 ch341 / cp210x 支持），
  本节可忽略。
